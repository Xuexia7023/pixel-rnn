...
flags.DEFINE_integer("batch_size", 16, "size of a batch")

#128
flags.DEFINE_integer("hidden_dims", 129, "dimesion of hidden states of LSTM or Conv layers")
#15
flags.DEFINE_integer("recurrent_length", 2, "the length of LSTM or Conv layers")

flags.DEFINE_integer("out_hidden_dims", 33, "dimesion of hidden states of output Conv layers")
...
# data
flags.DEFINE_string("data", "cifar", "name of dataset [mnist, cifar]")
...
def main(_):
  ...
  if conf.data == "mnist":
    ...
    print "test_step_per_epoch  = {}".format(test_step_per_epoch)
    ...
    if conf.is_train:
      ...
      for epoch in iterator:
        ...
        for idx in xrange(train_step_per_epoch):
          images = binarize(next_train_batch(conf.batch_size)).reshape([conf.batch_size, height, width, channel])
        ...
        for idx in xrange(test_step_per_epoch):
          images = binarize(next_test_batch(conf.batch_size)).reshape([conf.batch_size, height, width, channel])
